 ...
 */
public class EfficientDocument_Copy extends Document {
	...
	public EfficientDocument_Copy(String text)
	{
		super(text);
		processText();
	}
	...
	{
		System.out.println("tok in isWord ="+tok);
	...
	private void processText()
	{
		...
		System.out.println("In Efficient Doc tokens = "+ tokens);
		//numWords=tokens.size();
		for (String token : tokens) {
			boolean isWord = isWord(token);
			System.out.println(" isWord status received for token ="+token + " "+isWord);
			if (isWord){
				numWords++;
	...
	testCase(new EfficientDocument_Copy("This is a test.  How many???  "
                + "Senteeeeeeeeeences are here... there should be 5!  Right?"),
                16, 13, 5);
    testCase(new EfficientDocument_Copy(""), 0, 0, 0);
    testCase(new EfficientDocument_Copy("sentence, with, lots, of, commas.!  "
                + "(And some poaren)).  The output is: 7.5."), 15, 11, 4);
    testCase(new EfficientDocument_Copy("many???  Senteeeeeeeeeences are"), 6, 3, 2); 
    testCase(new EfficientDocument_Copy("Here is a series of test sentences. Your program should "
				+ "find 3 sentences, 33 words, and 49 syllables. Not every word will have "
				+ "the correct amount of syllables (example, for example), "
				+ "but most of them will."), 49, 33, 3);
	testCase(new EfficientDocument_Copy("Segue"), 2, 1, 1);
	testCase(new EfficientDocument_Copy("Segue?"), 2, 1, 1);
	testCase(new EfficientDocument_Copy("Sentence"), 2, 1, 1);
	testCase(new EfficientDocument_Copy("Sentences?!"), 3, 1, 1);
	testCase(new EfficientDocument_Copy("Lorem ipsum dolor sit amet, qui ex choro quodsi moderatius, nam dolores explicari forensibus ad."), 32, 15, 1);
